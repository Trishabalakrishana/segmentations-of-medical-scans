...
class UNET(nn.Module):
    ...
    def forward(self, x):
        ...
        for idx in range(0, len(self.ups), 2):
            ...
            # Improved size handling using interpolation instead of cropping
...
class UNET_V2(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, features=[64, 128, 256, 512]):
        ...
        # Upsampling path (Decoder)
        for feature in reversed(features):
            # Use learnable upsampling for better reconstruction
            self.ups.append(
                nn.Sequential(
                    nn.ConvTranspose2d(feature * 2, feature, kernel_size=2, stride=2),
    ...
    def forward(self, x):
        ...
        for idx in range(0, len(self.ups), 2):
            ...
            # Better size handling using interpolation with proper mode
            if x.shape[2:] != skip_connection.shape[2:]:
                x = F.interpolate(x, size=skip_connection.shape[2:], mode='bilinear', align_corners=False)
...
def test():
    ...
    test_sizes = [(1, 3, 128, 128), (2, 3, 256, 256), (1, 3, 224, 224), (1, 3, 512, 512)]
...
def model_summary():
    ...
    total_params_v2 = count_parameters(model_v2)
    
    print(f"\n📊 Model Summary:")
    print(f"UNET - Total trainable parameters: {total_params:,}")
    print(f"UNET - Model size: ~{total_params * 4 / 1024 / 1024:.2f} MB (float32)")
    print(f"UNET_V2 - Total trainable parameters: {total_params_v2:,}")
    print(f"UNET_V2 - Model size: ~{total_params_v2 * 4 / 1024 / 1024:.2f} MB (float32)")
    
    # Memory usage estimation
    print(f"\n💾 Memory Usage Estimation (for 160x160 RGB input):")
